...
"""
    This document was slightly modified from the original version to line up
    with the PEP-8 Python Coding Style and other specific purposes
    Original Version:
...
# This class implements basic control functionality which we will be using in
# future tutorials.It can command takeoff/landing/emergency as well as drone
# movement It also tracks the drone state based on navdata feedback
...
from sensor_msgs.msg import NavSatFix as Fix
...
class BasicDroneController(object):
    def __init__(self, model):
        ...
        self.subNavdata = rospy.Subscriber('/'+model+'/navdata',Navdata,
                                           self.ReceiveNavdata)

        # Allow the controller to publish to the /ardrone/takeoff, land and
        # reset topics
        self.pubLand = rospy.Publisher('/'+model+'/land', Empty, queue_size=10)
        self.pubTakeoff = rospy.Publisher('/'+model+'/takeoff', Empty,
                                          queue_size=10)
        self.pubReset = rospy.Publisher('/'+model+'/reset', Empty,
                                        queue_size=10)
    ...
    def SendCommand(self,event):
        # The previously set command is then sent out periodically if the
        # drone is flying
