...
class Discontinuo(Controller):
    ...
    def leyControl(self,_):
        if self.start and self.t < self.repeat*self.T:
            ...
            rdx = m*(self.mxpp(
                self.t)-self.kx1*exp-self.kx2*ex)-m*self.Lxi1*copysign(1.0,
                                                                      sigma1)
            rdy = m*(self.mypp(
                self.t)-self.ky1*eyp-self.ky2*ey)-m*self.Lxi2*copysign(1.0,
                                                                      sigma2)
            rdz = m*(g+self.mzpp(
                self.t)-self.ky1*ezp-self.ky2*ez)-m*self.Lxi3*copysign(1.0,
                                                                      sigma3)
            ...
            self.z_velocity = -self.kz1*(self.zPos-self.mz(self.t))+self.mzp(
                self.t)
            ...
            if k > self.control_period:
                msg = 'Control law function taking more time ' \
...
if __name__=='__main__':
    ...
    controller = Discontinuo('ardrone', 2, 2, 3, ejes)
