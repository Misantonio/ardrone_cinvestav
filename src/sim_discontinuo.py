...
class Discontinuo(Controller):
    def __init__(self,model,num_widgets,rows,cols,titulos):
        super(Discontinuo,self).__init__(model,num_widgets,rows,cols,titulos)
        ...
        logger.info('Simulation: {}, Control Period: {}ms, '
                    'Maximum tilt: {}deg'.format(sim_num, self.control_period,
                                                 maxTilt))
        logger.info(
            'Gains: {}, Trajectory: {}'.format((self.kx1, self.kx2, self.ky1,
                                                self.ky2, self.kz1, self.kz2,
                                                self.Lxi1, self.Lxi2, self.Lxi3,
                                                self.kpa),
    ...
    def leyControl(self,_):
        if self.start and self.t < self.repeat*self.T:
            ...
            # Attitude Control
            sigma1 = self.kx1*ex+self.kx2*self.sum_ex+exp
            sigma2 = self.ky1*ey+self.ky2*self.sum_ey+eyp
            sigma3 = self.kz1*ez+self.kz2*self.sum_ez+ezp

            rdx = m*(self.mxpp(self.t)
                     - self.kx1*exp - self.ky2*ex) - m*self.Lxi1*copysign(
                     1.0, sigma1)
            rdy = m*(self.mypp(self.t)
                     - self.ky1*eyp - self.ky2*ey) - m*self.Lxi2*copysign(
                     1.0, sigma2)
            rdz = m*(g + self.mzpp(self.t)
                     - self.kz1*ezp - self.kz2*ez) - m*self.Lxi3*copysign(
                     1.0, sigma3)
            ...
            self.z_velocity = -self.kz1 * (self.zPos - self.mz(
                self.t)) + self.mzp(self.t)
...
if __name__=='__main__':
    ...
    controller = Discontinuo('ardrone_1', 2, 2, 3, ejes)
